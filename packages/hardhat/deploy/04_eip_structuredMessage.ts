...
const deployEIPs: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
  ...
  */
  ...
  const contract = await deploy("PatrickCollins", {
    from: deployer,
    log: true,
  ...
  try {
    ...
    await hre.run("verify:verify", {
      ...
      contract: "contracts/EIPs/StructuredMessage/PatrickCollins.sol:PatrickCollins",
    });
  } catch (error: any) {
...
deployEIPs.tags = ["eip", "structured", "712"];
