...
const DataDisplay: React.FC<DataDisplayProps> = ({ returnData, setReturnData, displayError }) => {
  ...
  const getValues = async () => {
    ...
    const contract = chainData[chain?.id]?.personalMessage?.main;
    if (!contract) {
    ...
    if (!value?.data) {
      console.log("Ruby balance is undefined!");
      // displayError("Ruby balance is undefined!");
      return;
    }
    console.log("Ruby balance:", value);
    console.log("Ruby balance number:", parseInt(value?.data.toString()));
    setValues({ userAddress: address, rubyBalance: parseInt(value.data) });
    ...
    <>
      ...
      <div className="mb-4 text-center text-2xl flex justify-center gap-4 flex-col">
        <div className="text-gray-400">
          ...
          <span className="ml-2 mr-1 text-gray-500">Ruby Balance:</span>
          {formatEther(BigInt(values.rubyBalance))}
        </div>
